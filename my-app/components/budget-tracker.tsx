...
const COLORS = ['#80b8ff ', '#80e0d0 ', '#ffebcc ', '#ffd0b2 ', '#cdc9f0 ', '#c2e6d1 ', '#ffe3b2 ', '#c6e9f2 ', '#d2f0b8 ', '#f0f2b2 ', '#ffa07a'];

const currencies = [
  { code: 'EGP', symbol: 'E£' }, // Added EGP at the top
  { code: 'USD', symbol: '$' },
  { code: 'EUR', symbol: '€' },
  { code: 'GBP', symbol: '£' },
]
...
export function BudgetTracker() {
  ...
  const installmentsByDescription = installmentEntries.reduce((acc, entry) => {
    if (!acc[entry.description]) {
      acc[entry.description] = {
        installmentId: entry.installmentId || 0, // Default to 0 if undefined
        totalAmount: entry.installmentAmount || 0, // Default to 0 if undefined
        paidAmount: 0,
  ...
  return (
    <div className="container mx-auto p-4">
      {showIntroModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md">
            ...
            <CardContent>
              <p className="mb-4">
                Cashii helps you manage your income and expenses, set financial goals, and track your spending habits.
      ...
      <div className="bg-[#121212] rounded-md p-4 flex justify-between items-center mb-8">
        ...
        <div className="flex items-center space-x-4">
          <Select value={currency.code} onValueChange={(value) => setCurrency(currencies.find(c => c.code === value) || currencies[0])}>
            <SelectTrigger className="w-[120px] text-white"> {/* Increased width and kept trigger text white */}
          ...
          <Button variant="outline" size="icon" onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>
            {theme === "dark" ? <Sun className="h-[1.2rem] w-[1.2rem] text-white" /> : <Moon className="h-[1.2rem] w-[1.2rem] text-white" />}
  ...
  <CardContent>
    {Object.entries(installmentsByDescription).length > 0 ? ( 
      <div className="space-y-4">
      ...
      <div className="h-24 flex items-center justify-center text-center">
        No installments Added Yet. Add your first installment from expenses to get started!
      </div>
    )}
  </CardContent>
</Card>
